...
namespace ProjetAutoEcoleS4.Data
{
    internal class PlanningDAO
    {
        ...
        public List<string> RecupererPlanningDAOJournalier(DateTime date, int id)
        {
            ...
            {
                cn.Open();
                string sql = @"SELECT HOUR(DateHeureDebut), MINUTE(DateHeureDebut),HOUR(DateHeureFin), 
                            MINUTE(DateHeureFin), e.PrenomEleve,e.NomEleve
                            FROM Planning p
                            LEFT JOIN Eleve e ON p.ID_Eleve = e.ID_Eleve
                            JOIN Moniteur m ON p.ID_Moniteur = m.ID_Moniteur 
                            WHERE MONTH(DateHeureDebut) =" + date.Month+ 
                            " AND YEAR(DateHeureDebut)=" + date.Year+ 
                            " AND DAY(DateHeureDebut)="+date.Day+ " AND m.ID_Moniteur="+id+ 
                ...
                {
                    {
                        while (dr.Read())
                        {
                            liste.Add($"De {dr.GetInt32("HOUR(DateHeureDebut)")}:{dr.GetInt32("MINUTE(DateHeureDebut)")} " +
                                $"à {dr.GetInt32("HOUR(DateHeureFin)")}:{dr.GetInt32("MINUTE(DateHeureFin)")} | " +
                                $"Avec {dr.GetString("PrenomEleve")} {dr.GetString("NomEleve")}");
                        }
                    }
                }
                return liste;
